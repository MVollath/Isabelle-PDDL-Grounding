...
begin
  ...
  lemma (in wf_ast_domain) t_sig_split_b:
    "ast_domain.sig detype_dom =
  ...
  lemma (in wf_ast_domain) t_sig_some:
  ...
  lemma (in wf_ast_domain) t_preds_arity:
  ...
  lemma "\<forall>y \<in> snd ` set m. y = r \<Longrightarrow> x \<in> fst ` set m \<Longrightarrow> map_of m x = Some r"
    by (metis img_snd in_fst_imageE map_of_SomeD weak_map_of_SomeI)

  lemma "snd ` set m = {r} \<Longrightarrow> x \<in> fst ` set m \<Longrightarrow> map_of m x = Some r"
    by (metis emptyE img_snd in_fst_imageE insertE map_of_SomeD weak_map_of_SomeI)
  ...
  proof -
    ...
    ultimately show ?thesis using 1
      by (metis map_of_eq_None_iff option.distinct(1) t_entT)
  qed

  lemma t_entT_None:
    shows "map_of ents x = None \<longleftrightarrow> map_of (detype_ents ents) x = None" sorry

  lemma t_constT: "constT c \<noteq> None \<longleftrightarrow> ast_domain.constT detype_dom c = Some \<omega>"
    using t_entT_alt ast_domain.constT_def detype_dom_def by fastforce

  lemma "m1 x = Some y \<or> m1 x = None \<and> m2 x = Some y \<Longrightarrow> (m2 ++ m1) x = Some y"
    by (simp add: map_add_Some_iff)

  lemma (in ast_problem) t_objT: "objT c \<noteq> None \<longleftrightarrow> ast_problem.objT detype_prob c = Some \<omega>"
  proof -
    interpret p2 : ast_problem detype_prob .
    have o1: "objT = map_of (objects P) ++ constT" using objT_def by simp

    have "objT c \<noteq> None \<longleftrightarrow> constT c \<noteq> None \<or> constT c = None \<and> map_of (objects P) c \<noteq> None" using objT_def by auto
    also have "... \<longleftrightarrow> p2.constT c = Some \<omega> \<or> p2.constT c = None \<and> map_of (objects P) c \<noteq> None"
      using t_constT t_entT_None detype_prob_def sorry
    also have "... \<longleftrightarrow> p2.constT c = Some \<omega> \<or> p2.constT c = None \<and> map_of (objects detype_prob) c = Some \<omega>"
      using t_entT_alt detype_prob_def by fastforce
    also have "... \<longleftrightarrow> p2.constT c = Some \<omega> \<or> p2.constT c = None \<and> map_of (objects detype_prob) c = Some \<omega>"
      using t_entT_None detype_prob_def by fastforce
    also have "... \<longleftrightarrow> (map_of (objects detype_prob) ++ p2.constT) c = Some \<omega>" using map_add_Some_iff sorry
    (* also have "... \<longleftrightarrow> p2.objT c = Some \<omega>" sorry *)
  ...
  lemma t_cnstT_some:
    ...
    using assms t_entT by (metis map_of_eq_None_iff ty_term.simps(2))

  lemma t_varT_some:
    assumes "ty_term (map_of vars) cT (term.VAR x) \<noteq> None"
    shows "ty_term (map_of (detype_ents vars)) cT' (term.VAR x) = Some \<omega>"
    using assms t_entT by (metis map_of_eq_None_iff ty_term.simps(1))
  ...
  lemma (in wf_ast_domain) t_atom_wf:
    assumes "\<forall>e. tyt e \<noteq> None \<longrightarrow> tyt2 e = Some \<omega>"
      "wf_atom tyt a"
    shows "ast_domain.wf_atom detype_dom tyt2 a"
  proof -
    interpret d2 : ast_domain detype_dom .
  ...
  lemma (in wf_ast_domain) t_fmla_wf:
    assumes "\<forall>e. tyt e \<noteq> None \<longrightarrow> tyt2 e = Some \<omega>"
      "wf_fmla tyt \<phi>"
    shows "ast_domain.wf_fmla detype_dom tyt2 \<phi>"
  ...
  lemma (in wf_ast_domain) t_fmla_atom_wf:
    assumes "\<forall>e. tyt e \<noteq> None \<longrightarrow> tyt2 e = Some \<omega>"
      "wf_fmla_atom tyt \<phi>"
    shows "ast_domain.wf_fmla_atom detype_dom tyt2 \<phi>"
    using assms
  ...
  lemma (in wf_ast_domain) t_eff_wf:
    assumes "\<forall>e. tyt e \<noteq> None \<longrightarrow> tyt2 e = Some \<omega>"
      "wf_effect tyt \<epsilon>"
    shows "ast_domain.wf_effect detype_dom tyt2 \<epsilon>"
  ...
  lemma t_ac_tyt:
    ...
    shows "ty_term (map_of (parameters (detype_ac a))) (ast_domain.constT detype_dom) x = Some \<omega>"
...
(* disj_fmlas (map (Atom \<circ> (\<lambda>p. predAtm p [term.VAR v])) (type_conds T)) *)
  ...
  lemma "\<forall>ent. snd (detype_ent ent) = \<omega>"
    by simp
  ...
  lemma set_type_names: "set type_names = insert ''object'' (fst ` set (types D))" by auto
  ...
  lemma params_ts_exist: (* somehow this isn't trivial for the solver *)
    ...
    using assms params_ts_wf set_type_names
    by (metis wf_type_iff_listed insertI2 type.exhaust_sel)

  lemma (in wf_ast_domain) type_precond_wf: (* TODO simplify if possible *)
    assumes "wf_action_params a" "p \<in> set (parameters a)"
    shows "ast_domain.wf_fmla detype_dom
      (ty_term (map_of (parameters (detype_ac a))) egal)
      (type_precond p)"
  proof -
    interpret d2 : ast_domain detype_dom .
    let ?tyt = "ty_term (map_of (parameters a)) egal"
    ...
    hence 1: "?tyt2 (term.VAR (fst p)) = Some \<omega>" using t_varT_some
      by (simp add: detype_ac_def)
    ...
    proof
      ...
      moreover have "set type_preds \<subseteq> set (predicates detype_dom)" using detype_dom_def by simp
      ultimately have "PredDecl (pred_for_type t) [\<omega>] \<in> set (predicates detype_dom)" by auto
  ...
  lemma (in wf_ast_domain) t_param_precond_wf:
  ...
  lemma (in restr_domain) t_acs_wf:
  ...
  theorem (in restr_domain) detype_dom_wf:
...
lemma (in wf_ast_problem) t_init_dis: "distinct (init detype_prob)"
  sorry

thm wf_ast_domain.t_fmla_wf

theorem (in restr_problem) detype_prob_wf:
  shows "ast_problem.wf_problem detype_prob"
proof -
  interpret p2 : ast_problem detype_prob .

  note p2.wf_problem_def
  note detype_prob_def
  ...
  have c3: "\<forall>(n, y)\<in>set (objects detype_prob). p2.wf_type y" using t_ents_wf detype_prob_def by fastforce
  note c4 = t_init_dis
  have c5: "p2.wf_world_model (set (init detype_prob))" sorry

  have c6a: "\<forall>e. objT e \<noteq> None \<longrightarrow> p2.objT e = Some \<omega>" sorry
  have c6b: "wf_fmla objT (goal detype_prob)" using detype_prob_def wf_prob_def by simp
  have c6: "p2.wf_fmla p2.objT (goal detype_prob)"
...
lemma (in restr_domain) "ast_domain.wf_domain detype_dom"
  by (rule detype_dom_wf)

lemma (in restr_problem) "ast_problem.wf_problem detype_prob"
  by (rule detype_prob_wf)
